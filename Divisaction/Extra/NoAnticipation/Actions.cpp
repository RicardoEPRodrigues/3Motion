    ...
    namespace NoAnticipation {
        ...
        Follow::Follow(std::weak_ptr<IAgent> agent) : Action() {
            ...
            if (auto agentshr = agent.lock()) {
                auto anticipation = make_shared<TimeProgressiveStage>(string("follows " + agentshr->getName()), 2000,
                                                                      7000);
                this->setStage(StageType::ANTICIPATION_INTERRUPTIBLE, anticipation);

                auto finished = make_shared<TimeProgressiveStage>(string("reaches " + agentshr->getName()), 2000, 3000);
...
//            this->setStage(StageType::CANCEL, cancel);
        ...
        LongWalk::LongWalk() : Action() {
            ...
            auto anticipation = make_shared<TimeProgressiveStage>(string("crosses the bridge"), 2000, 7000);
            this->setStage(StageType::ANTICIPATION_INTERRUPTIBLE, anticipation);

            auto finished = make_shared<TimeProgressiveStage>(string("reaches the end of the bridge"), 2000, 3000);
            this->setStage(StageType::FOLLOW_THROUGH, finished);
...
//        this->setStage(StageType::CANCEL, cancel);
        ...
        ThrowBall::ThrowBall() {
            ...
            auto anticipation = make_shared<TimeProgressiveStage>(string("throws the ball"), 2000, 7000);
            this->setStage(StageType::ANTICIPATION_INTERRUPTIBLE, anticipation);
//
...
//        this->setStage(StageType::ANTICIPATION_UNINTERRUPTIBLE, execution);
            ...
            auto finished = make_shared<TimeProgressiveStage>(string("sees the ball hit the target"), 2000, 3000);
            this->setStage(StageType::FOLLOW_THROUGH, finished);
